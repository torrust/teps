...
`A'_client (dh) e_server = a'_common`

9. Server untweaks the Tweaked User Authentication Public Key:
`A'_user - e_common = A_user`

10. Server checks the Tweaked Authentication Code:</br>
`A'_auth == a'_common (hash) URI_server`

11. Server checks the URI:</br>
`URI_server == URI_server`
12. Server finds the User Authentication Public Key in its database and confirms the authentication:</br>
`if "A_user" is good, authentcate for the client: "E_client"`
